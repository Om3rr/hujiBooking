app.controller('tableCtrl', ['$scope',function($scope) {
    ...
    $scope.init = function(){
        Promise.all([$scope.getRooms(), $scope.getSlots()]).then(function(resp){
            $scope.rooms = resp[0].data;
            $scope.slots = resp[1].data;
            $scope.maxFrame = $scope.rooms.map(function(room){return room.r_slots}).reduce(function(a, b) {
                return Math.max(a, b);
            });
